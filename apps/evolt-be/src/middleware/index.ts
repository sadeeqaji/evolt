...
type JwtInvestorPayload = JwtBase & {
  role?: 'investor';
  accountId?: string;
  investorId?: string;
};
...
export async function authenticateAdmin(
  ...
  try {
    // This middleware assumes authenticateUser has already run
    // and populated req.user with { id, role }
    const user = (req as any).user;

    if (!user || user.role !== 'admin') {
      throw httpErrors.unauthorized('Administrator access required');
    }

    // User is already attached by authenticateUser, we're just validating the role.
  } catch (err) {
    return reply.send(err);
  }
}
