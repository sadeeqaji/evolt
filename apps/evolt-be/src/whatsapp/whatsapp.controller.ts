...
export class WhatsAppController {
  ...
  /** Handle normal incoming WhatsApp messages */
  ...
  /** Handle encrypted WhatsApp Flow webhook (PIN entry, etc.) */
  handleFlowWebhook = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
      if (!this.isRequestSignatureValid(req)) return reply.status(432).send();

      const body = req.body as DecryptRequestBody;
      const decryptedRequest = decryptRequestBody(
        body,
      ...
      const screenResponse =
        (await this.whatsappService.getNextScreen(decryptedBody as DecryptedResponse)) || {};
